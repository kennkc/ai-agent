...
**Python 版本**：**当前使用 3.12.13**（`services\python\venv` 由 3.12 建立，与 CI 一致）。
最低支持 3.10，但 3.10 下 torch 被夹在 `>=2.6,<2.8`（原因见启用指南 §步骤 1.1）。
一键建环境时可显式指定解释器：
`pwsh -File scripts/setup-python-env.ps1 -PythonExe "C:\Users\<you>\.local\bin\python3.12.exe"`。
