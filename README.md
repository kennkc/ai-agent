...
| 分支 | 用途与约定 |
|---|---|
| `codex/phase0-1-hardening` | 本机主开发分支（Phase 0/1 加固 + Phase 2 观测区），日常改动先落这里 |
| `dev` | **另一台远端开发机的工作分支**。本机向它推送前必须先 `git fetch` 确认对方进度，避免覆盖对方的进行中工作 |
| `workbuddy/main` | 外部协作分支，**保留、不做整体合流**。需要其中内容时按「时间 + 语义」逐项摘取，不覆盖式合并 |

> 2026-09-16 决策：`workbuddy/main` 长期保留为独立线；`dev` 归属另一台开发机，本机仅在明确要求时向其推送。
