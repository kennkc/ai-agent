...
`web/work-platform` 是 Agent-Lifeform 的统一前台入口，承载三区 14 模块：

- 生命体区：总览、生命体征、决策沙盘、五感矩阵、进化视图、协作总线
- 工作台区：任务中心、任务对话、专家团队、技能市场、连接器、自动化、灵感案例
- 治理区：免疫审批
- 全局层：全局搜索、通知中心、偏好设置、生命体征迷你条、Mock/API 数据源切换
...
| 模块 | 当前展示能力 | 后续真实接入 |
|------|--------------|--------------|
| 总览 | 五体征摘要、四指标卡、团队协作活动图（多泳道并行执行）、模型调用趋势、模型运行状态、AI 优化建议、服务健康、成长时间轴 | overview 聚合 API、`wp.model.metrics`、`wp.optimization.suggestion` |
...
| 协作总线 | Agent 心跳、DAG 编排、MC-P 协议体、工件层、验收门 | MC bus + WS |
