...
class GlobalExceptionHandlerTest {
    ...
    void unmappedRouteMapsTo404NotServerError() {
        ResponseEntity<Map<String, Object>> response = handler.handleNoRoute(
                new NoResourceFoundException(HttpMethod.GET, "api/session/definitely-missing"));
    ...
    void noHandlerFoundAlsoMapsTo404() {
        ResponseEntity<Map<String, Object>> response = handler.handleNoRoute(
                new NoHandlerFoundException("GET", "/api/session/nope", new HttpHeaders()));
    ...
    void genuineServerFaultStillMapsTo500() {
        ResponseEntity<Map<String, Object>> response = handler.handleUnknown(new IllegalStateException("会话存储异常"));
